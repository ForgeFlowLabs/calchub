...
// ══════════════════════════════════════════════════════════════
//  AI ASK FIRST — user asks question, AI answers, then calculate
// ══════════════════════════════════════════════════════════════
...
.ai-ask-title { font-size: 0.95rem; font-weight: 700; color: #1f2937; }
.ai-ask-sub   { font-size: 0.75rem; color: #7c3aed; margin-top: 2px; font-weight: 500; }
.ai-ask-examples {
...
.ai-ask-chip:hover { background: #ddd6fe; border-color: #a78bfa; }
.ai-ask-input-row {
...
.ai-ask-input:focus { border-color: #7c3aed; box-shadow: 0 0 0 3px rgba(124,58,237,0.1); }
.ai-ask-btn {
...
.ai-ask-btn:hover { transform: translateY(-1px); box-shadow: 0 4px 16px rgba(124,58,237,0.4); }
.ai-ask-btn:disabled { opacity:0.65; cursor:not-allowed; transform:none; }
.ai-answer-box {
  margin-top: 1rem;
  background: #fff;
  border: 1.5px solid #c4b5fd;
...
.ai-answer-box.show { display: block; }
...
.ai-answer-label {
  ...
  text-transform: uppercase; letter-spacing: 0.08em;
  display: flex; align-items: center; gap: 6px; margin-bottom: 0.65rem;
}
.ai-answer-text {
  font-size: 0.9rem; color: #374151; line-height: 1.8;
}
.ai-answer-text strong { color: #111827; }
.ai-answer-footer {
...
.ai-spinner {
  ...
  animation: aiSpin 0.65s linear infinite; display: inline-block;
}
@keyframes aiSpin { to { transform:rotate(360deg); } }
`;

function injectAIBoxStyles() {
  if (document.getElementById('ai-box-styles')) return;
  const s = document.createElement('style');
  s.id = 'ai-box-styles';
  s.textContent = AI_BOX_STYLES;
  document.head.appendChild(s);
}

// ── MAIN FUNCTION: inject the ask-first AI box ────────────────
// Place above the calculator form
// containerId = where to inject
// calcName    = e.g. "Mortgage"
// examples    = array of suggested questions
function injectAIAskBox(containerId, calcName, examples) {
  injectAIBoxStyles();
  const container = document.getElementById(containerId);
  if (!container || container.querySelector('.ai-ask-box')) return;

  const chips = (examples || []).map(e =>
    `<button class="ai-ask-chip" onclick="this.closest('.ai-ask-box').querySelector('.ai-ask-input').value=this.textContent">${e}</button>`
  ).join('');

  container.innerHTML = `
    <div class="ai-ask-box">
      <div class="ai-ask-header">
        <div class="ai-ask-icon">✨</div>
        <div>
          <div class="ai-ask-title">Ask AI about ${calcName}</div>
          <div class="ai-ask-sub">Get a plain-English answer before you calculate</div>
        </div>
      </div>
      <div class="ai-ask-examples">${chips}</div>
      <div class="ai-ask-input-row">
        <textarea class="ai-ask-input" id="ai-ask-input-${containerId}"
          placeholder="Ask anything… e.g. 'Can I afford a $400k home on $80k salary?'"
          rows="1"
          onkeydown="if(event.key==='Enter'&&!event.shiftKey){event.preventDefault();askAI('${containerId}','${calcName}')}"
          oninput="this.style.height='auto';this.style.height=Math.min(this.scrollHeight,100)+'px'"
        ></textarea>
        <button class="ai-ask-btn" id="ai-ask-btn-${containerId}"
          onclick="askAI('${containerId}','${calcName}')">
          Ask ✨
        </button>
      </div>
      <div class="ai-answer-box" id="ai-answer-${containerId}"></div>
    </div>`;
}

async function askAI(containerId, calcName) {
  const input  = document.getElementById(`ai-ask-input-${containerId}`);
  const btn    = document.getElementById(`ai-ask-btn-${containerId}`);
  const answer = document.getElementById(`ai-answer-${containerId}`);
  ...
  const prompt = `You are a helpful, friendly expert on ${calcName} calculations.

A user asks: "${q}"
Currency context: ${cur.code}

Answer in plain English, 2–3 short paragraphs maximum. Be direct, specific and practical.
- If it involves numbers, give concrete examples with their currency
- If it's advice, be balanced and honest  
- End with one actionable tip
- Do NOT use bullet points or headers — flowing sentences only
- Keep it under 150 words total`;
  ...
  try {
    ...
    answer.innerHTML = `
      ...
      <div class="ai-answer-text">${result
        .replace(/\*\*(.*?)\*\*/g, '<strong>$1</strong>')
        .replace(/\*(.*?)\*/g, '<em>$1</em>')
        .trim()
      }</div>
      <div class="ai-answer-footer">⚡ Powered by Google Gemini · For guidance only, not professional advice</div>`;
    answer.classList.add('show');
  } catch(e) {
    answer.innerHTML = `<div class="ai-answer-text" style="color:#dc2626;">AI unavailable right now. Please try again in a moment.</div>`;
    answer.classList.add('show');
  }
...
// Keep injectExplainButton as a no-op so old calls don't break
function injectExplainButton() {}
function injectProblemBox(containerId, calcName, examples) {
  injectAIAskBox(containerId, calcName, examples);
}
